...
const Sellers = () => {
  ...
  const [updateForm, setUpdateForm] = useState<Seller>({ email: '', password: '' });
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [idDeleteModal, setIdDeleteModal] = useState(undefined);
  const [userData, setUserData] = useState<Seller[]>([]);
  ...
  function showUpdateModel(seller: Seller) {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        const response = await urls.getSellers()
  ...
  async function updateUsers() {
    await urls.updateSellers(updateForm).then(async (response) => {
      console.log(response.data);
      closeUpdateModel();
      const updatedUserResponse = await urls.getSellerById(updateForm.id);
  ...
  async function deleteSellers(id) {
    try {
      await urls.deleteSellers(id);  
      ...
      <div className=" overflow-auto sm:rounded-lg   ml-16">
        <div className='flex  justify-between'>
          <div className='relative w-full max-w-96 mt-3 mb-5'>
            <h1 className='font-semibold text-3xl'>Sellers</h1>
          </div>
          <div className="relative w-full max-w-96 mt-3 mb-5">
          ...
          {paginatedData.map((seller) => (
            <tbody key={seller?.id}>
              <tr className="bg-white border-b">
                ...
                <td className="px-6 py-4">
                  <button onClick={() => showDeleteModel(seller.id)} className="text-white bg-gradient-to-br from-pink-500 to-orange-400 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-pink-200 font-medium rounded-lg text-sm px-5 py-2 text-center ">Delete</button>
                </td>
                <td className="px-6 py-4">
                  <button onClick={() => showUpdateModel(seller)} className="text-white bg-gradient-to-br from-purple-600 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2 text-center  ">Update</button>
                </td>
              </tr>
            </tbody>
          ))}
        ...
        <div id="deleteModal" tabIndex={-1} aria-hidden="true" className=" justify-center  grid content-center fixed top-0 overflow-y-auto overflow-x-hidden right-0 left-0 z-50  items-center w-full md:inset-0 md:h-full">
          <div className="relative p-4 w-full max-w-md h-full md:h-auto">
            <div className="relative p-4 text-center bg-white rounded-lg shadow sm:p-5">
              ...
              <p className="mb-4 text-gray-500">Are you sure you want to delete this seller?</p>
              <div className="flex justify-center items-center space-x-4">
                ...
                <button onClick={() => deleteSellers(idDeleteModal)} type="submit" className="py-2 px-3 text-sm font-medium text-center text-white bg-red-600 rounded-lg hover:bg-red-700 focus:ring-4 focus:outline-none focus:ring-red-300">
                  Yes, I'm sure
                </button>
              </div>
            </div>
            ...
            <div className="w-[800px] relative bg-white rounded-lg shadow">
              ...
              <form onSubmit={updateSubmit}>
                <div className="relative px-5 z-0 w-full mb-6 group mt-8">
                  <label htmlFor="email-input-Update" className="block mb-2 text-sm font-medium text-gray-900 ">Email</label>
                  <input id="email-input-Update" name="email" value={updateForm.email || ''} onChange={updateChange} placeholder="Write email here..." type="text" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" />
                </div>
                <div className="relative px-5 z-0 w-full mb-6 group mt-8">
